...
class ICacheModule(outer: ICache) extends LazyModuleImp(outer)
  ...
  // @todo what's the difference between [[s1_tag_disparity]] and [[s1_tl_error]]?
  /** wire indicate that tag is corrupt.
    * will trigger CPU replay and I$ invalidating.
    */
  val s1_tag_disparity = Wire(Vec(nWays, Bool()))
  /** wire indicates that bus has error.
    * @todo respond to CPU [[io.resp.bits.ae]], cause [[Causes.fetch_access]].
    */
  val s1_tl_error = Wire(Vec(nWays, Bool()))
  /** how many bits will be fetched for each fetch. */
  val wordBits = outer.icacheParams.fetchBytes*8
  /** a set of data read from [[data_arrays]]. */
  ...
  /** init data SRAM banks,
    ...
    * indexed with physical memory address and multi-beats cycle.
